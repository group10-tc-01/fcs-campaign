...
namespace Fcg.Campaign.Application.Abstractions.Behaviors;
    ...
    where TRequest : notnull
{
    ...
    public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
    {
        ...
        {
            stopwatch.Stop();
            _logger.LogError(exception, "Application request {RequestName} failed after {ElapsedMs} ms", requestName, stopwatch.ElapsedMilliseconds);
            throw;
        }
    }
    ...
    private void LogCompletion(string requestName, TResponse response, long elapsedMs)
    {
        ...
        if (result is null)
        {
            _logger.LogInformation("Application request {RequestName} completed after {ElapsedMs} ms", requestName, elapsedMs);
            return;
        }

        if (result.IsSuccess)
        {
            _logger.LogInformation("Application request {RequestName} completed successfully after {ElapsedMs} ms", requestName, elapsedMs);
            return;
        }

        _logger.LogWarning(
            "Application request {RequestName} failed with {ErrorCode} after {ElapsedMs} ms: {ErrorMessage}",
            requestName,
            result.Error.Code,
            elapsedMs,
